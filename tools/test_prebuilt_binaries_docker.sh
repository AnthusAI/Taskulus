...
kanbusr_bin=${1:-"${repo_root}/dist/kbs"}
console_bin=${2:-"${repo_root}/dist/kbsc"}
...
if [[ ! -f "$kanbusr_bin" ]]; then
  echo "kbs binary not found at $kanbusr_bin" >&2
  exit 1
fi
if [[ ! -f "$console_bin" ]]; then
  echo "kbsc binary not found at $console_bin" >&2
  exit 1
fi
