...
mod tests {
    ...
    #[test]
    fn format_issue_key_handles_numeric_and_suffixes() {
        assert_eq!(format_issue_key("12345", false), "12345");
        assert_eq!(format_issue_key("kanbus-abcdef", false), "kanbus-abcdef");
        assert_eq!(format_issue_key("kanbus-abcdef", true), "abcdef");
        assert_eq!(
            format_issue_key("kanbus-abcdef.more", false),
            "kanbus-abcdef.more"
        );
        assert_eq!(format_issue_key("kanbus-abcdef.more", true), "abcdef.more");
    }

    #[test]
    fn format_issue_key_handles_missing_key_segment() {
        assert_eq!(format_issue_key("-abcdef", false), "abcdef");
        assert_eq!(format_issue_key("kanbus-", false), "kanbus");
    }

    #[test]
    fn generate_issue_identifier_uses_test_uuid_sequence() {
        let sequence = vec![Uuid::nil()];
        set_test_uuid_sequence(Some(sequence));
        let request = IssueIdentifierRequest {
            title: "Title".to_string(),
            existing_ids: HashSet::new(),
            prefix: "kanbus".to_string(),
        };
        let result = generate_issue_identifier(&request).unwrap();
        assert_eq!(
            result.identifier,
            "kanbus-00000000-0000-0000-0000-000000000000"
        );
        set_test_uuid_sequence(None);
    }

    #[test]
    fn generate_many_identifiers_returns_unique_set() {
        let ids = generate_many_identifiers("Title", "kanbus", 3).unwrap();
        assert_eq!(ids.len(), 3);
    }
}
